...
5) Настал новый день, и первое, с чего начинаем -- пишем `git pull` чтобы проверить, не появилось ли никаких изменений. И снова выбираем нужную ветку, работаем, добавляем изменения, пушим изменения. Объединять ветки уже буду самостоятельно. **ВАЖНО!** Предварительно нужно выбрать веткуб прежде чем выполнить pull, обычно сигналом о том, что всё гуд, является сообщение "Обновление..." буквально в пару строк
6) Все задания, за которые *взялись*, помечаем в списке задач знаком (+). Если задача выполнена, то ставим на ней ✅. Также вот прям совсем идеально было бы создавать md файлы с описанием всего того, что и как делали. Для себя любимых же, чтобы фиксировать полученный опыт

== На данный момент выполнены все работы (вроде как) по предобработке данных. Расположены в ветке develop ==

Ну, вроде бы всё. В чате не материмся, мам любим и ценим, приступаем

==На данный момент выполнены все базовые работы. Остаётся только оформление, улучшение моделей
и подключение различных сервисов для красоты (прометей, алертменеджер, графана)== 
